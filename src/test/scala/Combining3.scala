...
//    if (how ne JoinType.RIGHT) columns.addAll(left.getColumns)
//    else  right.getColumns.map(col => columns.append(col))
//
...
//        else Collections.nCopies[V](right.getColumns.size, null))
...
//          Collections.nCopies[V](if (how ne JoinType.RIGHT) left.getColumns.size
...
//  }
//
//  @SafeVarargs def concat[V](first: DataFrame[V], others: DataFrame[? <: V]*): DataFrame[V] = {
//    val dfs = new  ListBuffer[DataFrame[? <: V]]()//others.length + 1)
//    dfs.append(first)
//    dfs.addAll(others)
//    var rows = 0
