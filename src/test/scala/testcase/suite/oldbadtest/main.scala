...
//    println(df2.values[Float].printArray())
  ...
  val testPath =
    "D:\\data\\git\\testNumpy\\src\\main\\resources\\avazu\\test.csv"
  //    val dff = readCSV
  val df2 = DataFrame.readCSV(testPath, 50)
  val df = DataFrame.readCsv(testPath, 2000000000)
  println(
    s"df column size : ${df.getColumns.size} ,col ->" +
      df.getColumns.mkString(","),
  )
  val selectColz = Array(
    "C15",
    "C14",
    "C17",
    "C16",
  ...
  val numCols =
    Array("C21", "site_domain", "device_model", "C18", "site_category")
  println(
    s"ccdf column size : ${cdf.getColumns.size} ,col ->" +
      cdf.getColumns.mkString(","),
  )
//    cdf.show()
//    df.show()
...
def mainssz(): Unit = {
  val npFile2 =
    "D:\\data\\git\\testNumpy\\src\\main\\resources\\npy_dir\\random_bool_array.npy"
...
//    (1 to 5).map(println)
  ...
  val cols: Seq[String] =
    Seq("category", "name", "value", "version", "age", "score")
  val rows: Seq[String] = Seq("row1", "row2", "row3", "row4", "row5", "row6")
  val col1Data = Seq("test", "release", "alpha", "beta", "gama", "peter")
  val col2Data = Seq("one", "two", "three", "four", "five", "six")
  val col3Data = Seq(14, 25, Float.NaN, 0, 52, 67)
