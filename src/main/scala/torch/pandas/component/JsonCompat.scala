...
object JsonCompat {
        ...
        try
          if (index < limit || limit == -1) {
            ...
            if (index % 1000 == 0) {
              ...
              logger.debug(s"json read progress $index time cost all: $duration s ,this duration time cost $zduration s")
              preTmpEndTime = endTime
            }
          } else {
            logger.info(s"json read finish! data count $index")
            break
          }
        catch {
          case e: Exception =>
            logger.error(s"parse json line $line meet error: ${e.getMessage}")
        }
      }
    }
  ...
  /** *
    ...
    * @param limit
    *   // val value = item(header) match { // case Str(s) => s // case Num(n) =>
    *   n // case Bool(b) => b // case Null => null // case Arr(_) =>
    *   item(header).toString // case Obj(_) => item(header).toString // case
    *   other => other.toString // }
    * @return
    */
  def parseJsonLineToColumnMap(
      jsonPath: String,
    ...
    val headers =
      if !recursionHeader then data.head.obj.keys
      else data.flatMap(_.obj.keys).toSet
  ...
  /** *
    ...
    * @return
    */
  def parseJsonLineToDataFrame(
      jsonPath: String,
      recursionHeader: Boolean = false,
      limit: Int = -1,
  ): DataFrame[AnyRef] = {
    val jsonStrArrayMap =
      parseJsonLineToColumnMap(jsonPath, recursionHeader, limit)
    val dataAsSeq = jsonStrArrayMap.values.map(_.toSeq).toList
    val count = jsonStrArrayMap.values.head.length
    new DataFrame[AnyRef](
      (0 to count).map(_.toString).toSeq,
      jsonStrArrayMap.keys.toSeq,
      dataAsSeq,
    )
  }
...
//    val jsonStr = Files.readString(Paths.get(jsonPath))
  ...
  /** *
    ...
    * @return
    */
  def parseJsonFileToCaseClassSeq[T](
      jsonPath: String,
      jsonConvFunc: ujson.Value => T,
    ...
    try
      ...
      case e: Exception =>
        logger.error(s"read json file to case class seq error: ${e.getMessage}")
        Seq.empty[T]
  }
