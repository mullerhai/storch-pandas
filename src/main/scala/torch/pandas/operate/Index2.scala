...
//import storch.DataFrame.RowFunction
//
//class Index(private val indexMap: mutable.LinkedHashMap[Any, Int] = mutable.LinkedHashMap.empty) {
//  def this(names: Iterable[Any]) = this(mutable.LinkedHashMap.from(names.zipWithIndex))
//  def this(names: Iterable[Any], size: Int) = this {
//    val map = mutable.LinkedHashMap.empty[Any, Int]
...
//      throw IllegalArgumentException(s"duplicate name '$name' in index")
//    }
//    indexMap(name) = value
//
...
//    indexMap.getOrElse(name, throw IllegalArgumentException(s"name '$name' not in index"))
//
...
//
//  def self: mutable.LinkedHashMap[Any, Int] = indexMap
//}
//
//object Index {
...
//        new RowFunction[V, Any] {
//          override def apply(values: List[V]): List[List[Any]] =
//            val key = cols.map(values).toList
//            List(List(key))
//        }
//      }
//    }
