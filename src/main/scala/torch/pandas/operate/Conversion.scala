...
object Conversion {
  ...
  def convert[V](df: DataFrame[V], numDefault: DataFrame.NumberDefault, naString: String): Unit = {
    ...
    numDefault match {
      case LONG_DEFAULT =>
        converters = List[DataFrame.Function[V, ?]](new Conversion.LongConversion[V], new Conversion.DoubleConversion[V], new Conversion.BooleanConversion[V], new Conversion.DateTimeConversion[V])
      case DOUBLE_DEFAULT =>
        converters = List[DataFrame.Function[V, ?]](new Conversion.DoubleConversion[V], new Conversion.LongConversion[V], new Conversion.BooleanConversion[V], new Conversion.DateTimeConversion[V])
      case _ =>
        throw new IllegalArgumentException("Number default contains an Illegal value")
    }
    val naConverter = new Conversion.NAConversion[V](naString)
    // find conversions
    for (c <- 0 until cols) {
  ...
  @SafeVarargs def convert[V](df: DataFrame[V], columnTypes: Class[? <: V]*): Unit = {
    ...
    for (i <- 0 until columnTypes.length) {
      ...
      if (cls != null) {
        ...
        if (classOf[Date].isAssignableFrom(cls)) conv = new Conversion.DateTimeConversion[V]
        else if (classOf[Boolean].isAssignableFrom(cls)) conv = new Conversion.BooleanConversion[V]
        else if (classOf[Long].isAssignableFrom(cls)) conv = new Conversion.LongConversion[V]
        else if (classOf[Number].isAssignableFrom(cls)) conv = new Conversion.DoubleConversion[V]
        else if (classOf[String].isAssignableFrom(cls)) conv = new Conversion.StringConversion[V]
  ...
  @SuppressWarnings(Array("rawtypes", "unchecked")) protected def variableToDummy[V](colVals:  Seq[V], extra:  Seq[AnyRef], columnName: String, references: LinkedHashMap[String, String], naString: String): Conversion.VariableToDummyResult = {
    ...
    new Conversion.VariableToDummyResult(result.toSeq, names)
  }
