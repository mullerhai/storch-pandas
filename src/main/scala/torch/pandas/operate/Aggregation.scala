...
import org.apache.commons.math3.stat.descriptive.moment.{Kurtosis as KKurtosis, Mean as KMean, Skewness as KSkewness, StandardDeviation as KStandardDeviation, Variance as KVariance}
...
object Aggregation {
  ...
  class Unique[V] extends DataFrame.Aggregate[V, V] {
    override def apply(values:  Seq[V]): V = {
      ...
      if (unique.size > 1) throw new IllegalArgumentException("values not unique: " + unique)
      values.head
    }
  }

  class Collapse[V](private val delimiter: String =",") extends DataFrame.Aggregate[V, String] {
    def this ={
      this(",")
    }
  ...
  abstract  class AbstractStorelessStatistic[V] protected(protected val stat: StorelessUnivariateStatistic) extends DataFrame.Aggregate[V, Number] {
...
//          else 0
  ...
  class Product[V] extends Aggregation.AbstractStorelessStatistic[V](new KProduct) {
  }

  class Mean[V] extends Aggregation.AbstractStorelessStatistic[V](new KMean) {
  }

  class StdDev[V] extends Aggregation.AbstractStorelessStatistic[V](new KStandardDeviation) {
  }

  class Variance[V] extends Aggregation.AbstractStorelessStatistic[V](new KVariance) {
  }

  class Skew[V] extends Aggregation.AbstractStorelessStatistic[V](new KSkewness) {
  }

  class Kurtosis[V] extends Aggregation.AbstractStorelessStatistic[V](new KKurtosis) {
  }
  ...
  abstract  class AbstractStatistic[V] protected(protected val stat: UnivariateStatistic) extends DataFrame.Aggregate[V, Number] {
    override def apply(values:  Seq[V]): Number = {
      var count = 0
...
//      }
  ...
  class Percentile[V](quantile: Double) extends Aggregation.AbstractStatistic[V](new KPercentile(quantile)) {
  }

  class Describe[V] extends DataFrame.Aggregate[V, StatisticalSummary] {
    final private val stat = new SummaryStatistics
...
//      }
  ...
  private def name(df: DataFrame[?], row: AnyRef, stat: AnyRef) = {
    ...
    if (df.index.size > 1) List(row, stat)
    else stat
  }
          ...
          if (!desc.columns.contains(col)) {
            ...
            if (desc.isEmpty) {
              ...
              for (r <- df.getIndex) {
                ...
                for (stat <- List("count", "mean", "std", "var", "max", "min")) {
                  val name = name(df, r, stat)
                  desc.append(name, Seq[V])
                }
              }
            }
          }
