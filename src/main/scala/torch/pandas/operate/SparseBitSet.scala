...
object SparseBitSet {
  ...
  def parameters: String = {
    ...
    sb.append(String.format("%s parameters:\n", classOf[SparseBitSet].getName))
      .append(String.format(
        "size:\tlevel 1=%d\tlevel 2=%d\tlevel 3=%d\n",
        L1_SIZE,
        L2_SIZE,
        L3_SIZE,
      )).append(String.format(
        "bits:\tlevel 1=%d\tlevel 2=%d\tlevel 3=%d\n",
        L1_BITS,
        L2_BITS,
        L3_BITS,
      )).append(String.format(
        "shift:\tlevel 1=%d\tlevel 2=%d\tlevel 3=%d\n",
        L1_SHIFT,
        L2_SHIFT,
        L3_SHIFT,
      )).append(String.format(
        "mask:\tlevel 1=%s\tlevel 2=%s\tlevel 3=%s\n",
        java.lang.Integer.toHexString(L1_MASK),
        java.lang.Integer.toHexString(L2_MASK),
        java.lang.Integer.toHexString(L3_MASK),
      ))
    sb.toString
  }
}
...
class SparseBitSet {
  private var bits = new Array[Array[Array[Long]]](
    SparseBitSet.L3_SIZE / SparseBitSet.INDEX_FACTOR,
  )
  ...
  def get(index: Int): Boolean = {
    ...
    if (
      index < 0 ||
      l1i < bits.length && (bits(l1i) != null && bits(l1i)(l2i) != null)
  ...
  def set(index: Int, value: Boolean): Unit = {
    ...
    if (value) {
      if (bits.length <= l1i && l1i < SparseBitSet.L1_SIZE) {
        val size = min(
          SparseBitSet.L1_SIZE,
          max(
            bits.length << 1,
            1 << 32 - java.lang.Integer.numberOfLeadingZeros(l1i),
          ),
        ) // int.size
        if (bits.length < size) bits = java.util.Arrays.copyOf(bits, size)
      }
      if (bits(l1i) == null)
        bits(l1i) = new Array[Array[Long]](SparseBitSet.L2_SIZE)
      if (bits(l1i)(l2i) == null)
        bits(l1i)(l2i) = new Array[Long](SparseBitSet.L3_SIZE)
  ...
  def set(index: Int): Unit = set(index, true)

  def set(start: Int, end: Int): Unit = for (i <- start until end) set(i)

  def clear(index: Int): Unit = set(index, false)

  def clear(start: Int, end: Int): Unit = for (i <- start until end) clear(i)

  def flip(index: Int): Unit = set(index, !get(index))

  def flip(start: Int, end: Int): Unit = for (i <- start until end) flip(i)
      ...
      while (bits(l1i) != null && l2i < bits(l1i).length) {
        while (bits(l1i)(l2i) != null && l3i < bits(l1i)(l2i).length) {
          ...
          if ((bits(l1i)(l2i)(l3i) & 1L << l4i) != 0L)
            return l1i << SparseBitSet.L1_SHIFT | l2i << SparseBitSet.L2_SHIFT |
              l3i << SparseBitSet.L3_SHIFT | l4i
          l3i += 1
          l4i = 0
        }
        l2i += 1
