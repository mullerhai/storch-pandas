...
object Shell {
  ...
  @throws[IOException]
  def repl(input: InputStream, frames:  Seq[DataFrame[AnyRef]]): AnyRef = new Shell.Repl(input, frames).run
  ...
  private object Repl {
    ...
    def source(ctx: Context, scriptable: Scriptable, args: Array[AnyRef], func: Function): Unit = {
      val repl = classOf[Shell.Repl].cast(scriptable)
      for (i <- 0 until args.length) {
        val file = Context.toString(args(i))
        val source = new Shell.Repl#SourceReader(file)
  ...
  @SerialVersionUID(1L)
  private class Repl private(private val input: InputStream, private val frames:  Seq[DataFrame[AnyRef]]) extends ScriptableObject {
    ...
    def run: AnyRef = {
      ...
      if (interactive) {
        val pkg = classOf[DataFrame[_]].getPackage
      ...
      try {
        ...
        for (cls <- classOf[DataFrame[_]].getDeclaredClasses) {
        ...
        var expr: String = null
        while (!quit && (expr = read(console)) != null) try {
          result = eval(expr)
          if (result ne Context.getUndefinedValue) {
            // store last value for reference
    ...
    @throws[IOException]
    def read(console: Shell.Repl#Console): String = {
    ...
    @SuppressWarnings(Array("unused")) def quit(): Unit = {
      quit = true
    }

    @throws[IOException]
    private def console(input: InputStream): Shell.Repl#Console = {
      if (interactive) try return new Shell.Repl#JLineConsole
      catch {
        case ignored: NoClassDefFoundError =>
      }
      new Shell.Repl#Console(new BufferedReader(new InputStreamReader(input)))
    }
    ...
    {
      this.reader = null
      final private var reader: BufferedReader = null
